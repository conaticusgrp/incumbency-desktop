export const RESIZE_BAR_SIZE = 5;                   // px
export const MIN_WINDOW_WIDTH = 400;                // px
export const MIN_WINDOW_HEIGHT = 200;               // px
export const WINDOW_HEADER_HEIGHT = 25;             // px

export const APP_LIST_WIDTH_PERCENT = 17;           // %
export const APP_LIST_MIN_WIDTH = 200;              // px
export const DATE_TIME_HEIGHT = 3;                  // em
export const TOOLBAR_HEIGHT = 3;                    // em

export const NOTIFICATION_WIDTH = 345;              // px
export const NOTIFICATION_HEIGHT = 133;             // px
export const NOTIFICATION_MARGIN_X = 2              // em
export const NOTIFICATION_MARGIN_Y = 1              // em

// DEBUG, to be used
export const KEEP_NOTIFICATIONS_DISPLAYED = false;
