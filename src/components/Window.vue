...
const unmaximize = (): void => {
    startTransition(0.2);
    pos.value = {
        x: boundsBeforeMaximizing.value.x,
        y: boundsBeforeMaximizing.value.y,
    };
    size.value = {
...
const handleResize = (e: MouseEvent): void => {
    if (resizeType.value?.w === "r") {
        ...
        const untilMinWidth =
            pos.value.x + size.value.width - MIN_WINDOW_WIDTH.value;
...
listen<{ severity: string; percent: number; unemployed_count: number }>(
    "unemployed_high",
    ({ payload }) => {
        if (payload.severity === "mild") {
            emailStore.createEmail(
                emails.ralph.highUneploymentEmail(
                    payload.percent,
